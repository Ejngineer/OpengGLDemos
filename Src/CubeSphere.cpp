...
void Cubesphere::SetUp()
{
	...
	glBufferData(GL_ARRAY_BUFFER, (unsigned int)interleavedVertices.size() * sizeof(float), getInterleavedVertices(), GL_STATIC_DRAW);
...
void Cubesphere::buildVerticesFlat()
{
	...
	float bt[6];                        //Tanget/BiTangent vector
	...
	for (unsigned int i = 0; i < vertexCountPerRow - 1; ++i)
	{
		...
		for (unsigned int j = 0; j < vertexCountPerRow - 1; ++j, ++k1, ++k2)
		{
			...
			Cubesphere::computeFaceTangents(v1, v2, v3, t1, t2, t3, bt);

			// add 4 vertex attributes
			addVertices(v1, v2, v3, v4);
			addNormals(n, n, n, n);
			addTexCoords(t1, t2, t3, t4);
			addTangents(bt, bt, bt, bt);
	...
	for (int i = 0, j = 0; i < vertexSize; i += 3, j += 2)
	{
		...
		addTangent(-tangents[i], tangents[i + 1], -tangents[i + 2]);
		addBiTangent(-bitangents[i], bitangents[i + 1], -bitangents[i + 2]);
	}
	...
	for (int i = 0, j = 0; i < vertexSize; i += 3, j += 2)
	{
		...
		addTangent(-tangents[i + 2], tangents[i], -tangents[i + 1]);
		addBiTangent(-bitangents[i + 2], bitangents[i], -bitangents[i + 1]);
	}
	for (int i = 0; i < indexSize; ++i)
	{
		indices.push_back(startIndex + indices[i]);
	...
	for (int i = 0, j = 0; i < vertexSize; i += 3, j += 2)
	{
		...
		addTangent(-tangents[i + 2], tangents[i + 1], tangents[i]);
		addBiTangent(-bitangents[i + 2], bitangents[i + 1], bitangents[i]);
	}
	for (int i = 0; i < indexSize; ++i)
	{
		indices.push_back(startIndex + indices[i]);
...
void Cubesphere::addTangent(const float tx, const float ty, const float tz)
{
...
void Cubesphere::addTangents(const float t1[6], const float t2[6], const float t3[6], const float t4[6])
{
	tangents.insert(tangents.end(), t1, t1 + 3);
	tangents.insert(tangents.end(), t2, t2 + 3);
	tangents.insert(tangents.end(), t3, t3 + 3);
	tangents.insert(tangents.end(), t4, t4 + 3);
	bitangents.insert(bitangents.end(), t1+3, t1 + 6);
	bitangents.insert(bitangents.end(), t2+3, t2 + 6);
	bitangents.insert(bitangents.end(), t3+3, t3 + 6);
	bitangents.insert(bitangents.end(), t4+3, t4 + 6);
}

void Cubesphere::addBiTangent(const float bx, const float by, const float bz)
{
	bitangents.push_back(bx);
	bitangents.push_back(by);
	bitangents.push_back(bz);
}

void Cubesphere::addBiTangents(const float b1[6], const float b2[6], const float b3[6], const float b4[6])
{
	bitangents.insert(bitangents.end(), b1, b1 + 3);
	bitangents.insert(bitangents.end(), b2, b2 + 3);
	bitangents.insert(bitangents.end(), b3, b3 + 3);
	bitangents.insert(bitangents.end(), b4, b4 + 3);
}
...
void Cubesphere::computeFaceTangents(const float v1[3], const float v2[3], const float v3[3], const float t1[2], const float t2[2], const float t3[2], float tangents[6])
{
